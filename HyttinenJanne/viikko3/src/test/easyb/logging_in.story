...
scenario "user can not login with incorrect password", {
    given 'command login selected', {
       userDao = new InMemoryUserDao()
       auth = new AuthenticationService(userDao)
       io = new StubIO("login", "pekka", "akkeAp") 
       app = new App(io, auth)
    }

    when 'a invalid username and password are entered', {
       app.run()
    }
...
scenario "nonexistent user can not login to ", {
    given 'command login selected', {
       userDao = new InMemoryUserDao()
       auth = new AuthenticationService(userDao)
       io = new StubIO("login", "eioo", "akkep") 
       app = new App(io, auth)
    }

    when 'a nonexistent username and password are entered', {
       app.run()
    }
